...
let viewTask = async () => {
    ...
    todoList.forEach((task, index) => {
        console.log(chalk.green(`${index}:`) + chalk.yellow(`${task}`));
    });
};
let deleteTask = async () => {
    ...
    // console.log(`\n\t Welcome to Delete Task: `)
    let del = todoList.splice(deletedTask.index, 1);
...
let updateTask = async () => {
    ...
    // console.log(`\n\t Welcome to Updated Task:`)
    todoList[update_task_index.index] = update_task_index.task;
    console.log(chalk.green(`\n new Task at index no. ${update_task_index.index} updated successfully [For updated list check option "View To-do List"]`));
};
main();
